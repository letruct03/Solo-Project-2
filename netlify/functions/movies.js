// Netlify serverless function for Movie Collection Manager
// Handles all CRUD operations and stores data in Netlify Blob Storage
...
// Helper to load movies from file
function loadMovies() {
  try {
...
function validateMovie(data) {
  ...
  if (data.releaseYear) {
    const year = parseInt(data.releaseYear);
  ...
  if (data.personalRating) {
    const rating = parseFloat(data.personalRating);
...
exports.handler = async (event, context) => {
  ...
  try {
    // GET /api/movies - Get all with pagination
    if (method === 'GET' && path === '') {
      ...
      if (statusFilter) {
        movies = movies.filter(m => m.watchStatus === statusFilter);
      }
    ...
    if (method === 'POST' && path === '') {
      ...
      const newMovie = {
        ...
        watchStatus: data.watchStatus || 'Want to Watch',
        personalRating: data.personalRating ? parseFloat(data.personalRating) : null,
...
function getSampleMovies() {
  return [
    ...
    {id: 7, title: "A Quiet Place: Day One", director: "Michael Sarnoski", releaseYear: 2024, genre: "Horror", runtime: 99, watchStatus: "Completed", personalRating: 7.8, reviewNotes: "Tense prequel showing the invasion's beginning.", dateAdded: "2024-06-28T10:00:00Z"},
    ...
    {id: 13, title: "Oppenheimer", director: "Christopher Nolan", releaseYear: 2024, genre: "Drama", runtime: 180, watchStatus: "Completed", personalRating: 9.5, reviewNotes: "Masterpiece. Cillian Murphy's performance is incredible.", dateAdded: "2024-01-15T10:00:00Z"},
    ...
    {id: 21, title: "Alien: Romulus", director: "Fede Álvarez", releaseYear: 2024, genre: "Horror", runtime: 119, watchStatus: "Completed", personalRating: 8.3, reviewNotes: "Return to form for the Alien franchise.", dateAdded: "2024-08-16T10:00:00Z"},
    ...
    {id: 26, title: "The Apprentice", director: "Ali Abbasi", releaseYear: 2024, genre: "Drama", runtime: 120, watchStatus: "Watching", personalRating: 7.6, reviewNotes: "Sebastian Stan's transformation is impressive.", dateAdded: "2024-10-11T10:00:00Z"},
